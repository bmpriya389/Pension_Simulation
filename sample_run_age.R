...
for(l1 in ca){    
    ...
    for(i in seq(1,100)){
      a1<-ifelse(l1==min_age,l1,sample(subset(e_a,e_a<l1),1)) # random sample value of entry age
      ...
      c1<-sample(sgr,1) # random sample value of salary growth rate
  ...
  a<-createSheet(sampleWorkBook,paste("Current Age",l1))
  addDataFrame(dat_sample,col.names=TRUE,sheet = a)
  dat_sample<-NULL
  #save(dat_sample,file='Simulate_sample'+a1+'.RData')  # saving the Rdata file
...
saveWorkbook(sampleWorkBook,"sampleRunAge.xlsx") # writing the dataframe to an excel file
